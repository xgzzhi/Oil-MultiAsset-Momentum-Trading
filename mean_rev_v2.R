################################################################################
#           ICLO100 MEAN REVERSION STRATEGY - IMPROVED VERSION                 #
#              ICE Gasoil Futures - Adaptive Threshold                         #
################################################################################
...
# Strategy parameters - ADJUSTED FOR SHORT TEST PERIOD
LOOKBACK_WINDOW   <- 60      # days for rolling mean/sd
ENTRY_THRESHOLD   <- 0.75    # LOWERED from 1.0 for more trades
EXIT_THRESHOLD    <- 0.0     # exit at mean
TRANSACTION_COST  <- 0.0010  # 10 bps per trade

# Grid search options
LOOKBACK_OPTIONS  <- c(30, 45, 60, 90)
THRESHOLD_OPTIONS <- c(0.5, 0.75, 1.0, 1.25)
...
count_trades <- function(positions) {
  ...
  sum(changes != 0) / 2  # Entry and exit = 1 trade
}
...
cat(strrep("=", 78), "\n")
cat("ICLO100 MEAN REVERSION STRATEGY - ICE Gasoil Futures\n")
cat(strrep("=", 78), "\n\n")
...
df <- df_raw %>%
  ...
  mutate(
    ...
    roll_mean = rollapply(log_return, width = LOOKBACK_WINDOW, 
                          FUN = mean, na.rm = TRUE, 
                          fill = NA, align = "right"),
    roll_sd = rollapply(log_return, width = LOOKBACK_WINDOW, 
                        FUN = sd, na.rm = TRUE, 
                        fill = NA, align = "right"),
    
    zscore = (log_return - roll_mean) / roll_sd,
...
cat("Z-score calculation requires", LOOKBACK_WINDOW, "days lookback\n\n")
...
# Check z-score distribution in each period
for (period_name in c("Training", "Validation", "Test")) {
  df_period <- df %>% filter(period == period_name)
  if (nrow(df_period) > 0) {
    z_valid <- df_period$zscore[!is.na(df_period$zscore)]
    cat(sprintf("%s Period (n=%d):\n", period_name, length(z_valid)))
    if (length(z_valid) > 0) {
      cat(sprintf("  Z-score range: [%.2f, %.2f]\n", min(z_valid), max(z_valid)))
      cat(sprintf("  Z-score mean:  %.3f, std: %.3f\n", mean(z_valid), sd(z_valid)))
      cat(sprintf("  Times |z| > %.2f: %d (%.1f%%)\n", 
                  ENTRY_THRESHOLD, 
                  sum(abs(z_valid) > ENTRY_THRESHOLD),
                  100 * mean(abs(z_valid) > ENTRY_THRESHOLD)))
...
df_test <- df %>% 
  filter(period == "Test") %>%
  select(date, close, log_return, zscore, position, strategy_return) %>%
  mutate(
    log_return = sprintf("%.4f", log_return),
    zscore = sprintf("%.2f", zscore),
    strategy_return = sprintf("%.4f", strategy_return)
  )
print(df_test, n = Inf, width = Inf)
cat("\n")
...
cat("Strategy Parameters:\n")
cat(sprintf("  Lookback Window: %d days\n", LOOKBACK_WINDOW))
cat(sprintf("  Entry Threshold: %.2f std devs\n", ENTRY_THRESHOLD))
cat(sprintf("  Exit Threshold: %.2f std devs\n", EXIT_THRESHOLD))
...
cat(strrep("=", 78), "\n")
cat("PARAMETER OPTIMIZATION ON VALIDATION PERIOD\n")
cat(strrep("=", 78), "\n\n")

cat("Testing", length(LOOKBACK_OPTIONS) * length(THRESHOLD_OPTIONS), 
...
for (i in 1:nrow(optimization_results)) {
  lb <- optimization_results$lookback[i]
  th <- optimization_results$threshold[i]
  ...
  df_opt <- df_raw %>%
    ...
    mutate(
      log_return = c(NA, diff(log(close))),
      roll_mean = rollapply(log_return, width = lb, FUN = mean, na.rm = TRUE, 
                            fill = NA, align = "right"),
      roll_sd = rollapply(log_return, width = lb, FUN = sd, na.rm = TRUE, 
                          fill = NA, align = "right"),
      zscore = (log_return - roll_mean) / roll_sd
    ) %>%
    filter(!is.na(log_return), 
  ...
  for (j in 1:nrow(df_opt)) {
    ...
    if (current_pos == 0) {
      if (z < -th) current_pos <- 1
      else if (z > th) current_pos <- -1
    } else if (current_pos == 1 && z > 0) {
      current_pos <- 0
    } else if (current_pos == -1 && z < 0) {
      current_pos <- 0
    }
...
cat("\nTop 10 parameter combinations (ranked by Sharpe ratio on validation):\n\n")
print(optimization_results %>% 
        select(lookback, threshold, sharpe_fmt, max_dd_fmt, win_rate_fmt, num_trades) %>%
        rename(Lookback = lookback, Threshold = threshold, 
...
trade_log <- df %>%
  ...
  select(date, close, zscore, position_lagged, log_return, strategy_return, cum_strategy)

write.csv(trade_log, "trade_log_test_period.csv", row.names = FALSE)

write.csv(df %>% select(date, close, log_return, zscore, position, 
                        position_lagged, strategy_return, cum_strategy, 
                        drawdown, period),
          "iclo_strategy_full_results.csv", row.names = FALSE)

cat("\n\nResults exported:\n")
cat("  - trade_log_test_period.csv\n")
cat("  - iclo_strategy_full_results.csv\n")
cat("  - 4 plot PNG files\n\n")

cat(strrep("=", 78), "\n")
cat("ANALYSIS COMPLETE\n")
cat(strrep("=", 78), "\n")
cat("\nRECOMMENDATION: Based on optimization results, consider using the\n")
cat("top-ranked parameter combination for your final analysis.\n")
