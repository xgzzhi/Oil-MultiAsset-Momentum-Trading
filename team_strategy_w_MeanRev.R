rm(list = ls())
setwd("/Users/liangbo/Desktop/QF_5210/project/group")
# -------------------------- Config ------------------------------
files <- list(
  PCAAQ03 = "PCAAQ03.csv",
  PPXDM00 = "PPXDM00.csv",
...
trade_cost       <- 0.0010      # per position change
...
# Mean-reversion parameters (REVISED)
mr_lookback      <- 12          # 1 year lookback (was 60)
mr_entry_thresh  <- 1.5         # entry threshold (was 2.0, lower = more aggressive)
mr_exit_thresh   <- 0.3         # exit threshold (was 0.5, lower = quicker exit)
mr_weight_range  <- c(0.05, 0.1, 0.15, 0.2)
...
roll_sd <- function(x, n){
  x <- as.numeric(x)
  if (is.na(n) || n <= 1) return(rep(NA_real_, length(x)))
...
make_mr_signal <- function(ret, lookback=60, entry_thresh=2.0, exit_thresh=0.5){
  # Calculate adaptive z-score for returns
  roll_mu <- roll_mean(ret, lookback)
  roll_sigma <- roll_sd(ret, lookback)
  
  # Z-score
  zscore_mr <- (ret - roll_mu) / (roll_sigma + 1e-8)
  
  # Generate mean-reversion signals
  mr_sig <- numeric(length(ret))
  current_pos <- 0
  
  for (i in 1:length(ret)){
    if (is.na(zscore_mr[i])){
      mr_sig[i] <- 0
      current_pos <- 0
      next
    }
    ...
    # Entry logic (opposite to momentum - fade the move)
    if (current_pos == 0){
      if (z < -entry_thresh){
        current_pos <- 1   # Buy after big drop
      } else if (z > entry_thresh){
        current_pos <- -1  # Sell after big rally
      }
    }
    # Exit logic
    else if (current_pos == 1 && z > exit_thresh){
...
for (nm in names(raw)){
  ...
  # Momentum signal (original)
  best <- pick_best_ma(df$Price, ret, nm)
  sig_momentum  <- make_signal(df$Price, best$s, best$l)
  if (auto_flip_sign && isTRUE(best$flip)) sig_momentum <- -sig_momentum
  
  # Mean-reversion signal (NEW)
  sig_mr <- make_mr_signal(ret, mr_lookback, mr_entry_thresh, mr_exit_thresh)
  
  prep_per_asset[[nm]] <- data.frame(
    Date       = df$Date,
    YM         = format(df$Date, "%Y-%m"),
    ret        = ret,
    sig_mom    = sig_momentum,
    sig_mr     = sig_mr,
    s          = best$s,
    l          = best$l,
    flipped    = best$flip,
...
merge_two <- function(a,b){
  merge(a[,c("YM","ret","sig_mom","sig_mr")], b[,c("YM","ret","sig_mom","sig_mr")],
        by="YM", all=FALSE, suffixes=c(".x",".y"))
}
tmp    <- merge_two(prep_per_asset[[1]], prep_per_asset[[2]])
merged <- merge(tmp, prep_per_asset[[3]][,c("YM","ret","sig_mom","sig_mr")], by="YM", all=FALSE)
names(merged) <- c("YM",
                   paste0(names(prep_per_asset)[1], c("_ret","_sig_mom","_sig_mr")),
                   paste0(names(prep_per_asset)[2], c("_ret","_sig_mom","_sig_mr")),
                   paste0(names(prep_per_asset)[3], c("_ret","_sig_mom","_sig_mr")))
...
for (nm in names(prep_per_asset)){
  ...
  sig_mom <- merged[[paste0(nm,"_sig_mom")]]
  pos     <- sig_mom
...
sig_mom_mat <- as.matrix(merged[, paste0(names(prep_per_asset), "_sig_mom"), drop=FALSE])
sig_mr_mat  <- as.matrix(merged[, paste0(names(prep_per_asset), "_sig_mr"), drop=FALSE])

ew_ret  <- rowMeans(ret_mat, na.rm = FALSE)
sig_sum <- rowSums(sig_mom_mat, na.rm=TRUE)

# Original: Consensus on equal-weight returns (momentum only)
...
# Original: Weighted-signal portfolio (inverse-vol weights, momentum only)
vols <- apply(ret_mat, 2, function(x) stats::sd(x, na.rm=TRUE))
w_iv <- 1/vols; w_iv[!is.finite(w_iv)] <- 0
w_iv <- w_iv / sum(w_iv)
sig_w  <- sweep(sig_mom_mat, 2, w_iv, `*`)
ws_turn <- c(0, rowSums(abs(diff(sig_w)), na.rm=TRUE))
ws_pnl  <- rowSums(sig_w * ret_mat, na.rm=TRUE) - trade_cost * ws_turn

# NEW: Test different mean-reversion weights
cat("\n========== DIAGNOSTIC: Mean-Reversion Signals ==========\n")
cat("Checking if MR signals are being generated...\n")
for (nm in names(prep_per_asset)){
  sig_mr_col <- paste0(nm, "_sig_mr")
  if (sig_mr_col %in% names(merged)){
    mr_vals <- merged[[sig_mr_col]]
    cat(sprintf("  %s MR signal: Long=%d, Short=%d, Flat=%d\n", 
                nm, sum(mr_vals == 1), sum(mr_vals == -1), sum(mr_vals == 0)))
  }
}
cat("====================================================\n\n")

hybrid_results <- list()
for (mr_w in mr_weight_range){
  mom_w <- 1 - mr_w
  
  # Combined signal: mostly momentum, small MR component
  sig_hybrid <- sweep(sig_mom_mat, 2, w_iv * mom_w, `*`) + 
    sweep(sig_mr_mat, 2, w_iv * mr_w, `*`)
  
  hybrid_turn <- c(0, rowSums(abs(diff(sig_hybrid)), na.rm=TRUE))
  hybrid_pnl  <- rowSums(sig_hybrid * ret_mat, na.rm=TRUE) - trade_cost * hybrid_turn
  
  # Debug: Check if PnL actually differs
  if (mr_w == 0.05){
    cat(sprintf("\nDEBUG Hybrid_MR%.0f%%:\n", mr_w*100))
    cat("  First 5 hybrid PnL values:", paste(sprintf("%.4f", head(hybrid_pnl, 5)), collapse=", "), "\n")
    cat("  Pure momentum PnL (for comparison):", paste(sprintf("%.4f", head(ws_pnl, 5)), collapse=", "), "\n")
  }
  
  hybrid_results[[sprintf("Hybrid_MR%.0f%%", mr_w*100)]] <- mk_metrics(hybrid_pnl)
}
...
# Diagnostic: Check Consensus portfolio
cat("\n========== DIAGNOSTIC: Consensus Portfolio ==========\n")
cat("Number of months:", length(cons_pnl), "\n")
cat("Non-zero returns:", sum(cons_pnl != 0), "\n")
cat("Consensus positions breakdown:\n")
cat("  Long (+1):", sum(cons_pos == 1), "\n")
cat("  Short (-1):", sum(cons_pos == -1), "\n")
cat("  Flat (0):", sum(cons_pos == 0), "\n")
cat("====================================================\n\n")

res <- list(
  PCAAQ03_momentum = mk_metrics(single_pnls$PCAAQ03),
  PPXDM00_momentum = mk_metrics(single_pnls$PPXDM00),
  PPXDK00_momentum = mk_metrics(single_pnls$PPXDK00),
  Consensus_EW_Portfolio = mk_metrics(cons_pnl),
  WeightedSignal_Momentum = mk_metrics(ws_pnl)
)

# Add hybrid results
res <- c(res, hybrid_results)
...
cat("========== Enhanced Three-Asset Strategy: Momentum + Mean-Reversion ==========\n")
cat(sprintf("cost=%.4f, winsor=%s @%.1f%%, spread_scale_win=%d, clamp=±%.0f%%\n",
            trade_cost, use_winsor, 100*winsor_p, spread_scale_win, 100*clamp_spread))
cat(sprintf("MR lookback=%d, entry_thresh=%.1f, exit_thresh=%.1f\n\n",
            mr_lookback, mr_entry_thresh, mr_exit_thresh))

cat("Momentum Parameters (MA crossover):\n")
for (nm in names(prep_per_asset)){
  s <- unique(prep_per_asset[[nm]]$s)
  l <- unique(prep_per_asset[[nm]]$l)
  f <- unique(prep_per_asset[[nm]]$flipped)
  cat(sprintf("  %-8s : short=%d, long=%d, flipped=%s\n", nm, s, l, f))
}
...
cat("\n========== Analysis ==========\n")
cat("Compare 'WeightedSignal_Momentum' (original, 100% momentum)\n")
cat("with 'Hybrid_MR5%', 'Hybrid_MR10%', etc. (momentum + mean-reversion blend)\n")
cat("\nNote: MR uses adaptive z-score threshold of", mr_entry_thresh, "\n")
cat("(higher threshold = less aggressive, more suitable as secondary factor)\n")
